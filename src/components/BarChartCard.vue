...
const rawData = ref<BarChartResponse | null>(null)
...
// y accessors — one per series, so VisGroupedBar renders a bar group per datum
const yAccessors = computed(() =>
  seriesKeys.value.map(k => (d: any) => d[k] as number)
)

// color accessor: i is the series index inside each bar group
const colorAccessor = computed(() => {
  const keys = seriesKeys.value
  return (_d: any, i: number) => `var(--color-${keys[i] ?? i})`
})
  ...
  return (_item: Record<string, string | number>, x: number | Date) => {
    ...
    const items = keys.map(key => {
      ...
      return `<div style="display:flex;align-items:center;gap:8px;padding:1px 0">
        <span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:var(--color-${key});flex-shrink:0"></span>
...
<template>
  <Card :class="hasError ? 'border-destructive/40 bg-destructive/5' : ''">
    ...
    <CardContent>
      ...
      <template v-else-if="!hasError && chartData.length && yAccessors.length">
